...
neomia_need() { command -v "$1" >/dev/null 2>&1 || return 1; }
...
ensure_gh_login() {
  ...
  gh auth status || neomia_err "Échec d'authentification GitHub CLI."
}
...
mode_ssh_account_key() {
  ...
  if gh auth status >/dev/null 2>&1; then
    ...
    gh ssh-key add "${SSH_KEY_PATH}.pub" -t "$title" || neomia_warn "Impossible d'ajouter automatiquement la clé (droits?). Ajoute-la manuellement."
  else
    neomia_warn "gh non authentifié : ajoute manuellement la clé publique dans GitHub > Settings > SSH and GPG keys."
    neomia_say "Clé publique : ${SSH_KEY_PATH}.pub"
  fi
...
mode_ssh_deploy_key() {
  ...
  local REPO_SLUG=""
  read -rp $'\033[1;35m[Neomia]\033[0m \033[1;36mEntrez le slug du dépôt (ex: neosaastech/neosaas-dev) : \033[0m' REPO_SLUG
  [[ -z "$REPO_SLUG" ]] && neomia_err "Slug de dépôt requis."
  ensure_ssh_config
  ensure_gh_login
  ...
  gh repo deploy-key add "${SSH_KEY_PATH}.pub" --repo "${REPO_SLUG}" -t "$title" --allow-write || \
    neomia_warn "Échec de l'ajout deploy-key. Vérifie que tu as les droits maintainer/admin sur ${REPO_SLUG}."
  neomia_say "Test SSH → github.com"
  ssh -T git@github.com || true
}
...
menu() {
  ...
  │          Neomia Studio - Menu          │
  └───────────────────────────────────────────┘\033[0m
  Sélectionne le mode de connexion GitHub :

  \033[1;36m[1]\033[0m SSH - Clé liée au COMPTE GitHub (accès global)
  \033[1;36m[2]\033[0m SSH - Deploy Key liée à un REPO spécifique
  \033[1;36m[3]\033[0m HTTPS + PAT via GitHub CLI

EOM
  read -rp $'\033[1;35m[Neomia]\033[0m \033[1;36mTon choix [1/2/3] : \033[0m' choice
  case "${choice:-1}" in
...
Utilise maintenant \033[1;36mgit\033[0m ou \033[1;36mgh\033[0m pour interagir avec GitHub.
