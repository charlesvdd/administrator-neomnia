...
#            React + Next.js sur Debian/Ubuntu, en root,
#            et créer le projet dans /opt/<nom_du_projet>
# ----------------------------------------------------

### 1. Auto-élévation vers root si nécessaire ###
if [ "$EUID" -ne 0 ]; then
  echo "⚠️  Vous n’êtes pas root. Relancement automatique avec sudo..."
  exec sudo bash "$0" "$@"
fi
...
### 5. Fonction : comparer deux versions (via dpkg) ###
version_ge() {
  # renvoie vrai si $1 >= $2
...
### 7. Installer Git, curl, build-essential si manquant ###
...
### 8. Vérifier / installer Node.js ≥ 18.x ###
...
if [ -d "${PROJECT_DIR}" ]; then
  ...
  if [ -f "package.json" ]; then
    ...
    # Vérification ou initialisation du dépôt Git
    if [ -d ".git" ]; then
      echo -e "${GREEN}   • Dépôt Git déjà initialisé. Vous pouvez 'git pull' si besoin.${RESET}"
    else
...
### 13. Création d’un nouveau projet Next.js dans /opt/<nom> ###
...
### 15. Initialisation du dépôt Git local ###
if [ -d ".git" ]; then
